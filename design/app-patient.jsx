...
function App() {
  ...
  const navProvider = []; // patient portal — no provider section
  const isInFlow = ['book','confirm','voicecall'].includes(screen);
  ...
  const tabItems = isPractice ? [
    ...
    { id:'discover',  label:'Symptom',  icon:<I.sparkle size={18}/> },
  ] : [
    { id:'landing',   label:'Home',     icon:<I.home size={18}/> },
          ...
          <div className="col gap-2" style={{marginTop: 8}}>
            ...
            {isInFlow && (
              <button className={`nav-item active`} style={{marginLeft:24, fontSize:13}}>
                ...
                <I.cal size={14}/>
                <span>{screen==='book'?'Booking…':screen==='voicecall'?'On call':'Confirmed'}</span>
              </button>
            )}
          </div>
    ...
    <>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSelect label="Screen" value={screen}
          options={isPractice
            ? ['landing','about','discover','book','confirm','dashboard','voicecall']
            : ['landing','discover','doctors','book','confirm','dashboard','voicecall']
          }
          onChange={(v)=> go(v)}/>
      </TweaksPanel>
    </>
